...
def create_user(db: Database, user_data: dict):
    ...
    new_user = {
        ...
        "timezone": "UTC",
        "auth_provider": "local",
        "tv_webhook_token": secrets.token_urlsafe(24),
...
def create_google_user(db: Database, google_data: dict):
    ...
    new_user = {
        ...
        # Generate a random password hash so the field is never empty/predictable
    ...
    db.users.insert_one(new_user)
    print(f"✅ Created Google user {new_user['email']} with permanent ID: {user_id}")
    return new_user
...
def delete_user_account(db: Database, user_id: str, password: str) -> bool:
    ...
    try:
        ...
        # Then scrub user identity but keep the record for the admin dashboard
        scrambled_email = f"deleted_{user_id}@journalx.com"
        db.users.update_one(
            {"user_id": user_id},
            {"$set": {
                ...
                "password": get_password_hash("NO_LOGIN_POSSIBLE_$$$"),
                "is_active": False,
                "role": "deleted",
                "mobile_number": "",
                "google_id": "",
        ...
        logger.info(f"Successfully deleted user account data and disabled account for {user_id}")
        return True
    except Exception as e:
