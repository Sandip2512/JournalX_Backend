...
from app.crud.user_crud import login_user, create_google_user
...
def google_login(
    ...
    try:
        ...
        if not db_user:
            # Create new user
            db_user = create_google_user(db, idinfo)
        else:
            # Ensure user_id exists (migration fallback)
            if "user_id" not in db_user:
                db_user["user_id"] = str(db_user["_id"])
            
            # Linking logic: Store google_id and update auth_provider if missing
            update_fields = {}
            if not db_user.get("google_id"):
                update_fields["google_id"] = idinfo.get("sub")
            if not db_user.get("auth_provider") or db_user.get("auth_provider") == "local":
                update_fields["auth_provider"] = "google"
            
            # Sync avatar if available
            if idinfo.get('picture') and db_user.get('avatar_url') != idinfo['picture']:
                update_fields["avatar_url"] = idinfo['picture']
            ...
            if update_fields:
                ...
                # Update local db_user object
                db_user.update(update_fields)

        # Record success
        background_tasks.add_task(log_login_history, db_user["user_id"], client_ip, "google_success", db)
