...
@router.get("/events/news/search")
async def get_event_news(
    query: str = Query(..., description="The query string to search for"),
    limit: int = Query(3, description="Number of articles to return")
):
    """
    Live scrape Google News RSS for macroeconomic impact
    """
    try:
        url = f"https://news.google.com/rss/search?q={quote_plus(query + ' economy market')}&hl=en-US&gl=US&ceid=US:en"
        ...
        for item in root.findall('.//item')[:limit]:
            ...
            # Clean up the title (Google sometimes appends " - Source Name" at the end)
            if " - " in title:
        ...
        # Return empty list rather than 500 to keep UI graceful
        return {
            "success": False,
            "news": []
        }
