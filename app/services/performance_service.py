...
class PerformanceService:
    ...
    def get_period_data(self, user_id: str, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        projection = {
            ...
            "followed_rules": 1,
            "followed_risk": 1,
            "followed_daily_loss": 1,
            "followed_trade_limit": 1,
            "_id": 0
        }
        ...
        stats = {
            ...
            "compliance": self._calculate_compliance_stats(df)
        }
    ...
    def generate_insights(self, df: pd.DataFrame, stats: Dict) -> Dict:
        ...
        if 'compliance' in stats:
            ...
            if comp['compliance_rate'] < 70:
                insights['weaknesses'].append(f"Low rule adherence ({comp['compliance_rate']}%). Discipline is your biggest edge.")
                if comp['violated_risk_count'] > comp['violated_daily_loss_count']:
                    insights['patterns'].append("Frequent risk management violations (sizing too large).")
                else:
                    insights['patterns'].append("Frequent overtrading or daily loss limit breaches.")
            elif comp['compliance_rate'] >= 90 and stats['total_trades'] >= 5:
                insights['strengths'].append("Excellent discipline and rule adherence.")

        return insights

    def _calculate_compliance_stats(self, df: pd.DataFrame) -> Dict[str, Any]:
        """Helper to aggregate rule compliance from trade flags."""
        if 'followed_rules' not in df.columns:
            return {"compliance_rate": 0, "total_violations": 0, "violated_risk_count": 0, "violated_daily_loss_count": 0, "followed_count": 0}
            
        total = len(df)
        followed = df[df['followed_rules'] == True]
        followed_count = len(followed)
        compliance_rate = (followed_count / total * 100) if total > 0 else 0
        
        # Breakdown
        violated_risk = len(df[df['followed_risk'] == False])
        violated_loss = len(df[df['followed_daily_loss'] == False])
        violated_limit = len(df[df['followed_trade_limit'] == False])
        
        return {
            "compliance_rate": round(compliance_rate, 1),
            "total_violations": total - followed_count,
            "followed_count": followed_count,
            "violated_risk_count": violated_risk,
            "violated_daily_loss_count": violated_loss,
            "violated_trade_limit_count": violated_limit
        }
