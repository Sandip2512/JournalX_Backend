...
def normalize_symbol(symbol: str) -> str:
    """
    Normalizes broker-specific trade symbols (e.g., 'XAUUSDm', 'EURUSD.c') 
    to a standard unified format ('XAU/USD') so stats aggregate correctly.
    """
    if not symbol:
        return ""
    ...
    # 1. Match standard Forex/Metals/Crypto pairs with common quotes
    quotes = r'(USD|EUR|GBP|JPY|AUD|NZD|CAD|CHF|USDT|BUSD|USDC)'
    # Matches base (3-5 chars) + optional sep + quote + optional suffix
    pattern = rf'^([A-Z]{{3,5}})[-_/\\]?{quotes}(?:[A-Z0-9.\-_]*)$'
    ...
    # 2. Cleanup indices / other instruments (e.g., US30.c -> US30, NAS100m -> NAS100)
    # Strip common broker punctuation endings
    s = re.sub(r'[\.\-_][A-Z0-9]+$', '', s)
    # Strip trailing "M" or "C" (mini/cent) if appended directly (e.g., US30M -> US30)
    s = re.sub(r'[MC]$', '', s)
