...
analytics_cache = {}
CACHE_DURATION_SECONDS = 30  # Short TTL so switching/disconnect reflects fast
...
def get_account_isolation_filter(db: Database, user_id: str) -> dict:
    ...
    if active_account:
        # Connected: show active MT5 account trades AND manual trades
        return {"$or": [
            {"mt5_account": active_account},
            {"mt5_account": raw_account},
            {"mt5_account": {"$in": [None, ""]}},
...
def calculate_analytics(db: Database, user_id: str) -> Dict[str, Any]:
    ...
    account_filter = get_account_isolation_filter(db, user_id)
...
def get_calendar_stats(db: Database, user_id: str, month: int, year: int) -> List[Dict]:
    ...
    query = {
        "user_id": user_id,
        "$and": [
            get_account_isolation_filter(db, user_id),
            {"$or": [
...
def get_diary_stats(db: Database, user_id: str, start_date: datetime, end_date: datetime) -> Dict:
    ...
    if sub_tier == "free":
        ...
        query["$and"] = [account_filter, date_cond]
    else:
        query.update(account_filter)
