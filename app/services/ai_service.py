...
class AIService:
    ...
    def _call_gemini_api(self, prompt: str) -> str:
        ...
        # Ensure we use the API key in the URL as it's often more reliable
        ...
        try:
            ...
            if response.status_code == 404:
                print(f"❌ Gemini API Error: Model not found or API disabled for this key. URL used: {url.split('?')[0]}")
                return "AI Service error (HTTP 404): The Gemini model was not found. Please ensure the 'Generative Language API' is enabled in your Google Cloud Console."
            
            if response.status_code == 403:
                print(f"❌ Gemini API Error: Permission denied. Key might be invalid or restricted.")
                return "AI Service error (HTTP 403): Permission denied. Your API key might be invalid, expired, or lacking 'Generative Language API' permissions."
            ...
            return "I couldn't generate a response. Please try again."
             ...
             return f"AI Service error (HTTP {response.status_code if 'response' in locals() else 'Unknown'})"
